...
class ProxyMiddleware(object):
    ...
    proxyfile = "../utils/validProxy.txt"
    ...
    def process_response(self, request, response, spider):
        ...
        elif response.status not in [200, 301, 302, 403, 404, 408, 500, 502, 503, 504]: #spider.website_possible_httpstatus_list:
...
class TopProxyMiddleware(ProxyMiddleware):
    ...
    def file_request(self):
        print('hello')
        return self._new_request_from_response(error_302.pop())
    ...
    def process_request(self, request, spider):
        ...
        if valid_proxy_list.__len__() > self.topindex:
            tenthLargestPencentageValue = sorted(valid_proxy_list ,key = lambda i: i.percentage)[-self.topindex].percentage 
        ...
        #If valided proxy length is below 8, choose randomly within them, if not it will meet index error
        else:
            request.meta['agent'] = random.choice(list(filter(lambda x: x.is_valid(),self.agent_list)))
            request.meta['proxy'] = request.meta['agent'].proxy
